  ...
  return (
    <div className="max-w-md mx-auto">
      ...
      <div className="text-center mb-10">
        <span className="text-5xl block mb-4">{experience.emoji}</span>
        <h1 className="font-display font-semibold text-2xl text-dark-brown mb-1.5 italic">
          {experience.title}
        </h1>
        <p className="text-muted-brown text-sm italic">{formatCents(remainingCents)} remaining</p>
      </div>

      {/* Step 1: Amount Selection */}
      {step === "amount" && allowSplit && (
        <div className="bg-white rounded-2xl border border-border-soft p-6 mb-6">
          <h2 className="font-display font-medium text-base text-dark-brown mb-4 italic">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-8 pr-4 py-3 rounded-xl bg-cream border border-border-soft text-dark-brown focus:border-rose focus:outline-none transition-colors"
            />
          </div>
          <p className="text-[11px] text-muted-brown mt-2 italic">
        ...
        <form onSubmit={handleSubmit}>
          <div className="bg-white rounded-2xl border border-border-soft p-6 mb-6">
            ...
            <input
              ...
              required
              className="w-full px-4 py-3 rounded-xl bg-cream border border-border-soft text-dark-brown focus:border-rose focus:outline-none transition-colors mb-5"
            />

            <label className="block mb-1.5 text-sm text-dark-brown">
              Personal Note <span className="text-muted-brown italic">(optional)</span>
            </label>
            <textarea
              value={note}
              ...
              rows={3}
              className="w-full px-4 py-3 rounded-xl bg-cream border border-border-soft text-dark-brown focus:border-rose focus:outline-none transition-colors resize-none"
            />
          </div>

          {error && (
            <div className="text-center text-rose text-sm mb-4 italic">{error}</div>
          )}
